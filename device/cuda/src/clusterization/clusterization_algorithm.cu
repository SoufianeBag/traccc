...
__global__ void ccl_kernel2(
    ...
    device::ccl_kernel2(threadIdx.x, blockDim.x, blockIdx.x, cells_view,
                       modules_view, max_cells_per_partition,
                       target_cells_per_partition, partition_start,
                       partition_end, outi, f, f_next, barry_r,
                       /*measurements_view, measurement_count,*/spacepoints_container, cell_links);
}
...
clusterization_algorithm2::output_type clusterization_algorithm2::operator()(
   ...
   /** alt_measurement_collection_types::buffer measurements_buffer(num_cells,
    ...
    //std::cout << *(spacepoints_container.size) << std::endl;
